    ...
    public void onRender(Render3DEvent event) {
        if (this.isEnabled()) {
            ...
            for (Entry<ItemData, Integer> itemEntry : itemMap.entrySet().stream().sorted((entry1, entry2) -> {
                ...
                double x = itemEntry.getKey().x - AccessorRenderManager.getRenderPosX(mc.getRenderManager());
                double y = itemEntry.getKey().y - AccessorRenderManager.getRenderPosY(mc.getRenderManager());
                double z = itemEntry.getKey().z - AccessorRenderManager.getRenderPosZ(mc.getRenderManager());
