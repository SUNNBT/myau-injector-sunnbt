        ...
        if (this.isEnabled()) {
            ...
            for (TileEntity chest : mc.theWorld.loadedTileEntityList.stream().filter(tileEntity -> tileEntity instanceof TileEntityChest || tileEntity instanceof TileEntityEnderChest).collect(Collectors.toList())) {
                ...
                AxisAlignedBB aabb = new AxisAlignedBB(
                        ...
                        .offset(
                                -AccessorRenderManager.getRenderPosX(mc.getRenderManager()),
                                -AccessorRenderManager.getRenderPosY(mc.getRenderManager()),
                                -AccessorRenderManager.getRenderPosZ(mc.getRenderManager())
                        );
                        ...
                        vec = new Vec3(0.0, 0.0, 0.0)
                                .rotatePitch(
                                        (float) (
                                                -Math.toRadians(
                                                        RenderUtil.lerpFloat(
                                                                mc.thePlayer.cameraPitch, mc.thePlayer.prevCameraPitch, AccessorMinecraft.getTimer(mc).renderPartialTicks
                                                        )
                                                )
                                        )
                                )
                                .rotateYaw(
                                        (float) (
                                                -Math.toRadians(
                                                        RenderUtil.lerpFloat(
                                                                mc.thePlayer.cameraYaw, mc.thePlayer.prevCameraYaw, AccessorMinecraft.getTimer(mc).renderPartialTicks
                                                        )
                                                )
                                        )
                                );
