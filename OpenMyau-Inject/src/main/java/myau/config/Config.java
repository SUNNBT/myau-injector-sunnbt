...
public class Config {
    ...
    public Config(String name, boolean newConfig) {
        ...
        try {
            ...
            if (newConfig) {
                AccessorMinecraft.getLogger(mc).info(String.format("Created: %s", this.file.getName()));
            }
        } catch (Exception e) {
            AccessorMinecraft.getLogger(mc).error(e.getMessage());
        }
    }
    ...
    public void load() {
        try {
                        ...
                        for (Property<?> property : list) {
                            if (object.has(property.getName())) {
                                try {
                                    ...
                                    AccessorMinecraft.getLogger(mc).warn(String.format("Failed to load property %s for module %s", property.getName(), module.getName()));
                                }
                            }
                        }
                    }
            ...
            AccessorMinecraft.getLogger(mc).error("JSON Syntax Error: " + e.getMessage());
        } catch (Exception e) {
            AccessorMinecraft.getLogger(mc).error("Error loading config: " + e.getMessage());
    ...
    public void save() {
        try {
                ...
                if (list != null) {
                    for (Property<?> property : list) {
                        try {
                            ...
                            AccessorMinecraft.getLogger(mc).warn(String.format("Failed to save property %s for module %s", property.getName(), module.getName()));
                        }
                    }
                }
            ...
            AccessorMinecraft.getLogger(mc).error("Error saving config: " + e.getMessage());
