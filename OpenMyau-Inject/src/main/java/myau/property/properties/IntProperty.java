package myau.property.properties;

import com.google.gson.JsonObject;
import myau.property.Property;

import java.util.function.BooleanSupplier;

public class IntProperty extends Property<Integer> {
    private final Integer minimum;
    private final Integer maximum;

    private final int step;

    public IntProperty(String name, Integer value, Integer minimum, Integer maximum) {
        this(name, value, minimum, maximum, 1, null);
    }

    public IntProperty(
            String name, Integer value, Integer minimum, Integer maximum, BooleanSupplier check
    ) {
        this(name, value, minimum, maximum, 1, check);
    }

    public IntProperty(String name, Integer value, Integer minimum, Integer maximum, int step) {
        this(name, value, minimum, maximum, step, null);
    }

    public IntProperty(
            String name, Integer value, Integer minimum, Integer maximum, int step,
            BooleanSupplier check
    ) {
        super(name, value, v -> v >= minimum && v <= maximum, check);
        this.step = Math.max(1, step);
        this.minimum = minimum;
        this.maximum = maximum;
    }

    public int getStep() {
        return this.step;
    }

    public int snap(int value) {
        long notches = Math.round((value - this.minimum) / (double) this.step);
        long snapped = this.minimum + notches * this.step;
        return (int) Math.max(this.minimum, Math.min(this.maximum, snapped));
    }

    @Override
    public String getValuePrompt() {
        return String.format("%d-%d", this.minimum, this.maximum);
    }

    @Override
    public String formatValue() {
        return String.format("&e%s", this.getValue());
    }

    @Override
    public boolean parseString(String string) {
        return this.setValue(Integer.parseInt(string));
    }

    @Override
    public boolean read(JsonObject jsonObject) {
        return this.setValue(jsonObject.get(this.getName()).getAsNumber().intValue());
    }

    @Override
    public void write(JsonObject jsonObject) {
        jsonObject.addProperty(this.getName(), this.getValue());
    }

    public Integer getMinimum() {
        return minimum;
    }

    public Integer getMaximum() {
        return maximum;
    }
}
