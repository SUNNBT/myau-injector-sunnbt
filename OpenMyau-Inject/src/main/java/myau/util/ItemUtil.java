...
import myau.access.AccessorItemSword;
...
public class ItemUtil {
    ...
    private static boolean isToolClass(ItemStack itemStack, String toolClass) {
        Item item = itemStack.getItem();
    ...
    public static int findInventorySlot(String toolClass, int startSlot, boolean checkDurability) {
        ...
        for (int i = 0; i < 36; ++i) {
            ...
            if (!ItemUtil.isToolClass(itemStack, toolClass)) continue;
            if (checkDurability) {
                if (itemStack.isItemDamaged()) {
                    if (itemStack.getMaxDamage() - itemStack.getItemDamage() < 30) {
                        continue;
    ...
    public static int findAndurilHotbarSlot(int currentSlot) {
        for (int i = currentSlot; i < currentSlot + 9; ++i) {
            ...
            if (itemStack.getItem() instanceof ItemSword && itemStack.hasTagCompound()) {
                ...
                if (AccessorItemSword.getMaterial(itemSword) == Item.ToolMaterial.IRON && itemStack.getTagCompound().hasKey("display", 10)) {
