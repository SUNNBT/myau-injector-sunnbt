...
public class GuiAccountManager extends GuiScreen {
    ...
    public void initGui() {
        ...
        buttonList.add(loginButton = new GuiButton(
                0, width / 2 - 150, height - 52, 72, 20, "Login"
        ));
        buttonList.add(new GuiButton(
                1, width / 2 - 76, height - 52, 72, 20, "Add"
        ));
        buttonList.add(new GuiButton(
                4, width / 2 - 2, height - 52, 72, 20, "Session"
        ));
        buttonList.add(new GuiButton(
                6, width / 2 + 72, height - 52, 73, 20, "Cookie"
        ));
                ...
                case 0: {
                    if (task == null || task.isDone()) {
                        ...
                        if (CookieAuth.MARKER.equals(account.getClientId())) {
                            ...
                            task = CookieAuth.relogin(account, executor, message -> notification = new Notification(
                                    TextFormatting.translate(message) + " (" + username + ")", -1L
                            )).thenRun(() -> notification = new Notification(TextFormatting.translate(String.format(
                                    "&aSuccessful login! (%s)&r", account.getUsername()
                            )), 5000L)).exceptionally(error -> {
                                notification = new Notification(TextFormatting.translate(String.format(
